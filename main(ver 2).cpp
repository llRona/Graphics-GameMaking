...
void handleBlockMovement(sf::RectangleShape& block, bool& movingRight, float blockSpeed) {
    if (movingRight) {
        ...
        if (block.getPosition().x + block.getSize().x >= windowWidth)
            movingRight = false;
    }
    else {
        block.move(-blockSpeed, 0);
...
bool handleBlockFalling(sf::RectangleShape& block, std::vector<sf::RectangleShape>& blocks, sf::View& view, float dropSpeed, int& score, float& viewYOffset, sf::RectangleShape& line) {
    ...
    for (auto& placedBlock : blocks) {
        if (block.getGlobalBounds().intersects(placedBlock.getGlobalBounds())) {
            ...
            if (newBlockWidth > 0) {
                block.setSize(sf::Vector2f(newBlockWidth, block.getSize().y));
                block.setPosition(overlapLeft, block.getPosition().y);
                blocks.push_back(block);
                block.setFillColor(getRandomColor());
                block.setPosition((windowWidth - block.getSize().x) / 2, viewYOffset);
    ...
    // 바닥과 충돌 시 처리
    if (block.getPosition().y + block.getSize().y >= windowHeight + viewYOffset - 5) {
        blocks.push_back(block);
        block.setFillColor(getRandomColor());
        block.setPosition((windowWidth - block.getSize().x) / 2, viewYOffset);
        viewYOffset -= blockSize;
...
bool runGame(sf::RenderWindow& window, sf::Font& font) {
    ...
    while (window.isOpen())
    {
        ...
        // 게임 오버 조건 확인
        if (falling && block.getPosition().y + block.getSize().y > line.getPosition().y) {
            break; // 게임 오버
        }
